...
internal static class ParticleCodeCodec
{
    public static string Serialize(ParticleConfig config)
    {
        ...
        Line(text, "render.mode", config.RendererMode);
        ...
        Line(text, "render.beamEnd", $"{N(config.BeamEndX)}, {N(config.BeamEndY)}, {N(config.BeamEndZ)}");
        Line(text, "render.beamNoise", config.BeamNoise);
        Line(text, "render.localSpace", config.LocalSpace);
        Line(text, "events.parent", Quote(config.ParentEmitterId));
        Line(text, "events.mask", config.ParentEvents);
        Line(text, "events.probability", config.EventProbability);
        Line(text, "events.count", config.EventSpawnCount);
        Line(text, "events.inheritVelocity", config.EventInheritVelocity);
        ...
        Line(text, "collision.radius", config.CollisionRadius);
        ...
        foreach (string raw in source.Replace("\r", string.Empty, StringComparison.Ordinal).Split('\n'))
        {
            ...
            try
            {
                ...
                switch (key)
                {
                    ...
                    case "render.mode": config.RendererMode = EnumValue<ParticleRendererMode>(value); break;
                    ...
                    case "render.beamend": (config.BeamEndX, config.BeamEndY, config.BeamEndZ) = Triple(value); break;
                    case "render.beamnoise": config.BeamNoise = Number(value); break;
                    case "render.localspace": config.LocalSpace = Boolean(value); break;
                    case "events.parent": config.ParentEmitterId = Unquote(value); break;
                    case "events.mask": config.ParentEvents = EnumValue<ParticleEventMask>(value); break;
                    case "events.probability": config.EventProbability = Number(value); break;
                    case "events.count": config.EventSpawnCount = Integer(value); break;
                    case "events.inheritvelocity": config.EventInheritVelocity = Number(value); break;
                    ...
                    case "collision.radius": config.CollisionRadius = Number(value); break;
