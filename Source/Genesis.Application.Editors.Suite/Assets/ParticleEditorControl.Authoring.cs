...
public sealed partial class ParticleEditorControl
{
    ...
    private static void ValidateParticleDraft(ParticleConfig config)
    {
        ...
        if (config.Speed < 0 || config.SpeedVariance is < 0 or > 1 || config.LifetimeVariance is < 0 or > .99
            ...
            || config.VelocityStretch < 0 || config.BeamNoise < 0
            || config.BoundsSizeX <= 0 || config.BoundsSizeY <= 0 || config.BoundsSizeZ <= 0)
            throw new FormatException("Lengths, rates and sizes must be non-negative; scales, trail/bounds sizes and ribbon limits must be positive; speed/lifetime variance must be in range.");
        ...
        if (!Enum.IsDefined(config.Shape) || !Enum.IsDefined(config.BlendMode) || !Enum.IsDefined(config.Alignment)
            || !Enum.IsDefined(config.RendererKind) || !Enum.IsDefined(config.SimulationSpace)
            || !Enum.IsDefined(config.BoundsMode) || !Enum.IsDefined(config.CollisionMode)
            || !Enum.IsDefined(config.SizeCurve) || !Enum.IsDefined(config.AlphaCurve))
    ...
    private void StepPreviewSimulations(float step) => StepPreviewExecution(step);
